...
class HistoricPartition:
    ...
    def _get_completed_job_information(self) -> str:
        ...
        return subprocess.run(['sacct', 
                               '--allusers', 
                               '--starttime', formatted_time_24_hours_ago, 
                               '--partition', self.name, 
    ...
    def _is_completed_data_line_valid(self, line_data: list) -> bool:
        ...
        if line_data[0] != 'COMPLETED' and line_data[0] != 'CANCELLED+' and line_data[0] != 'TIMEOUT' and line_data[0] != 'FAILED':
            return False
...
if __name__ == '__main__':
    ...
    manage_csv_file(parition.name, {'run_times': partition.run_times, 'requested_times': partition.requested_times})