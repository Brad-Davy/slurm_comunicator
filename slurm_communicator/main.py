...
if __name__ == "__main__":
    ...
    print("-" * int(0.7 * terminal_width))
    print(
        f"Currently {total_cores_in_use} cores are in use out of {total_cores_in_cluster} cores in the cluster."
    )
    print(
        f"Currently {(total_cores_in_use/total_cores_in_cluster)*100:.2f}/% of the cluster is in use."
    )
    print("-" * int(0.7 * terminal_width))
    print(
        f'There are {global_quantities["jobs_pending_in_the_queue"]} jobs pending in the queue.'
    )
    print(
